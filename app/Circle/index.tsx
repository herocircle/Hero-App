...
const CircleHomePage = ({ route, navigation }: props) => {
  ...
  const { data: currentWork } = useQuery({
    ...
    queryFn: async () => {
      const response = await new CircleWorkApi(AXIOS_CONFIG).getLatestWins(
        circleId,
        "en"
      );
      return response.data;
  ...
  const { data: circleData } = useQuery({
    ...
    queryFn: async () => {
      ...
      try {
        const response = await new CirclesApi(
          AXIOS_CONFIG
        ).getCircleIdByUrlName(circleUrlName as string);
  ...
  const {
    ...
    queryFn: async () => {
      const response = await new CircleHomeLandingApi(
        AXIOS_CONFIG
      ).getCircleByName(circleData?.circleName || "", "en");
      return response?.data?.[0];
    },
  });

  const { data: ambassadorsData } = useQuery({
    queryKey: ["circle-ambassadors"],
    queryFn: async () => {
      const response = await new AmbassadorApi(AXIOS_CONFIG).findAll();
      return response.data;
    },
  });

  const singleCircle = data?.filter(
    (item: { id: any }) => item.id === circleId
  )[0];
